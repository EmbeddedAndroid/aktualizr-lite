...
def invoke_aklite(options: List[str], kill_after_sec: Optional[float] = None ):
    ...
    if kill_after_sec is not None:
        proc = subprocess.Popen([aklite_path] + options, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        try:
...
def run_test_kill_process():
    ...
    try:
        ...
        for i in range(5):
            ...
            assert cp.returncode == -9, cp.stdout.decode("utf-8")
...
def test_forced_sync():
    ...
    blob_digest = out_json['indexes'][0]['blobs'][0]['digest']

    # Test forced `pull` command
    logger.info("Testing corruption of pulled blob, to make sure it's re-downloaded with `pull` command")
    pulled_blob_path = f"/var/sota/reset-apps/blobs/sha256/{blob_digest.split(':')[1]}"
    corrupt_file(pulled_blob_path, blob_digest.split(':')[1])
    cp = invoke_aklite(['pull', str(target.actual_version)])
    assert cp.returncode == ReturnCodes.Ok, cp.stdout.decode("utf-8")
    verify_file_integrity(pulled_blob_path, blob_digest.split(':')[1])
    ...
    installed_blob_path = f"/var/lib/docker/image/overlay2/imagedb/content/sha256/{blob_digest.split(':')[1]}"
    corrupt_file(installed_blob_path, blob_digest.split(':')[1])
    cp = invoke_aklite(['install', str(target.actual_version)])
    assert cp.returncode == ReturnCodes.Ok, cp.stdout.decode("utf-8")
    verify_file_integrity(installed_blob_path, blob_digest.split(':')[1])
